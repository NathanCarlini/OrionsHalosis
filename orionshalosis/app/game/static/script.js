...
function MyThree(props) {
    ...
    useEffect(() => {
        if (!scriptExecutedRef.current) {
            ...
            var socket = io(`https://orions-halosis.vercel.app`, { transports: ["websocket"] });
            let price = 0;
            let player1 = true;
            let player2 = false;
            let capt1x2 = false;
            ...
            function init() {
                ...
                function onDocumentKeyDown(event) {
                    if (event.key == "ArrowLeft" && recent == 0 || event.key == "q" && recent == 0) {
                        if (player1 == true) {
                    ...
                    if (event.key == "ArrowRight" && recent == 0 || event.key == "d" && recent == 0) {
                        for (let i = 0; i < planets.children.length; i++) {
                            if (player1 == true) {
                                whatPlanet()
                ...
                socket.on('turnPlayer', (m1, m2, p1, p2) => {
                    ...
                    if (player1 == true && firstPlayer == true) {
                        ...
                        text.innerHTML = "You are currently spectating "+usernameP1+"...";
                        document.removeEventListener("keydown", onDocumentKeyDown, false);
                        document.removeEventListener('mousedown', onMouseDown, false);
                    } else if (player2 == true && firstPlayer == false) {
                        turnP2++;
                        text.innerHTML = "Your turn "+usernameP2+"!<br>You are currently on " + uppercaseFirstLetter(currentPlanet.name);
                        document.addEventListener("keydown", onDocumentKeyDown, false);
                        document.addEventListener('mousedown', onMouseDown, false);
                    } else if (player2 == true && firstPlayer == true) {
                        text.innerHTML = "You are currently spectating "+usernameP2+"...";
                        document.removeEventListener("keydown", onDocumentKeyDown, false);
                        document.removeEventListener('mousedown', onMouseDown, false);
                    }
                })
